...
class DPMon():
    ...
    def volume_on_ip(self, ip, volume_direction="incoming", epsilon=1.0):
        
        if not volume_direction in {"incoming", "outgoing"}:
        ...
        return self.private_query(aggregation = f"sum(CASE WHEN {self.ipasn_col} == {ip} THEN {volume_col} ELSE 0 END)", \
                                  metric="sum", epsilon = 1.0)
